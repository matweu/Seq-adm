...
SeqAdm.exe надо запускать от имени Администратора, т.к программа собирает информацию с журнала безопасности.

Прототип представляет собой 2 блока программ, основной блок написан на C++, 
на Python реализована модель нейронной сети с LSTM сетью:
...
Сам прототип SeqAdm при работе запрашивает от пользователя информацию об обучении модели, 
...
К макету также прилагаются следующие файлы:
	...
	protatipefunc.cpp - Файлы с перечисляемыми типами, 
				вспомогательными функциями (Перевод в Unix дату и т.д)
